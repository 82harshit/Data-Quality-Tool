...
class DataQuality:
    ...
    def upsert_batch(self, batch_id: str, job_id: str, batch_date, data_quality_score: float, db_session: Session) -> None:
        """
        Inserts or updates a batch record. In case of an insuccessful insertion it will retry 3 times before throwing an error.
        ...
        while attempt < max_retries:
            try:
                ...
                db_session.rollback()
                attempt += 1
                error_msg = f"Error in upserting batch: {e}; Attempt: ({attempt}/{max_retries})"
                dqt_logger.error(error_msg)
                self.job_state.update_state_of_job_id(job_status=Job_Run_Status_Enum.ERROR, status_message=error_msg)
                if attempt < max_retries:
                    info_msg = f"Retrying in {delay} seconds..."
                    dqt_logger.info(info_msg)
                    self.job_state.update_state_of_job_id(job_status=Job_Run_Status_Enum.INPROGRESS, status_message=info_msg)
                    time.sleep(delay)
                else:
                    error_msg = "Max retries reached. Insert failed."
                    dqt_logger.error(error_msg)
                    self.job_state.update_state_of_job_id(job_status=Job_Run_Status_Enum.ERROR, status_message=error_msg)
 
    def insert_expectation(self, expectation_data: dict, db_session: Session) -> None:
        """
        Inserts a new expectation record. In case of an insuccessful insertion it will retry 3 times before throwing an error.
        ...
        while attempt < max_retries:
            try:    
                ...
                db_session.rollback()
                attempt += 1
                error_msg = f"Error in inserting expectation: {e}; Attempt: ({attempt}/{max_retries})"
                dqt_logger.error(error_msg)
                self.job_state.update_state_of_job_id(job_status=Job_Run_Status_Enum.ERROR, status_message=error_msg)
                if attempt < max_retries:
                    info_msg = f"Retrying in {delay} seconds..."
